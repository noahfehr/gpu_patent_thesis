...
class LDAConfig:
    ...
    predictions_path: Path = Path(
        "data/analysis/runs/v6__full__two_stage__ts1__predictions.jsonl"
    )

    # Path to the custom stopword list.
    # These are added on top of sklearn's built-in English stopwords.
    stopwords_path: Path = Path("custom_stopwords.txt")
    ...
    base_data_dir: Path = Path("data/claims_added")
    ...
    # Remove the top N most frequent vocabulary items globally before training.
    # Helps suppress generic boilerplate and overly common words.
    rm_top: int = 70
